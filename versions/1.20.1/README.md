# Folia 1.20.1 (Experimental)

This Docker image provides Folia Minecraft server version 1.20.1 (experimental build).
...
```bash
docker run -it -d -p 25565:25565 --name folia-1.20.1 -e MINECRAFT_EULA=true blackao/folia:1.20.1
```
...
```bash
docker build --build-arg VERSION=1.20.1 --build-arg BUILD=latest -t blackao/folia:1.20.1 versions/1.20.1/
```
