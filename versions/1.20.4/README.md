# Folia 1.20.4 (Experimental)

This Docker image provides Folia Minecraft server version 1.20.4 (experimental build).
...
```bash
docker run -it -d -p 25565:25565 --name folia-1.20.4 -e MINECRAFT_EULA=true blackao/folia:1.20.4
```
...
```bash
docker build --build-arg VERSION=1.20.4 --build-arg BUILD=latest -t blackao/folia:1.20.4 versions/1.20.4/
```
