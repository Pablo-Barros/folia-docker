# Folia 1.19.4 (Experimental)

This Docker image provides Folia Minecraft server version 1.19.4 (experimental build).
...
```bash
docker run -it -d -p 25565:25565 --name folia-1.19.4 -e MINECRAFT_EULA=true blackao/folia:1.19.4
```
...
```bash
docker build --build-arg VERSION=1.19.4 --build-arg BUILD=39 -t blackao/folia:1.19.4 versions/1.19.4/
```
