# Folia 1.21.5 (Experimental)

This Docker image provides Folia Minecraft server version 1.21.5 (experimental build).
...
```bash
docker run -it -d -p 25565:25565 --name folia-1.21.5 -e MINECRAFT_EULA=true blackao/folia:1.21.5
```
...
```bash
docker build --build-arg VERSION=1.21.5 --build-arg BUILD=latest -t blackao/folia:1.21.5 versions/1.21.5/
```
