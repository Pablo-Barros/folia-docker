# Folia 1.21.8 (Experimental)

This Docker image provides Folia Minecraft server version 1.21.8 (experimental build).
...
```bash
docker run -it -d -p 25565:25565 --name folia-1.21.8 -e MINECRAFT_EULA=true blackao/folia:1.21.8
```
...
```bash
docker build --build-arg VERSION=1.21.8 --build-arg BUILD=6 -t blackao/folia:1.21.8 versions/1.21.8/
```
