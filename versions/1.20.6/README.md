# Folia 1.20.6 (Experimental)

This Docker image provides Folia Minecraft server version 1.20.6 (experimental build).
...
```bash
docker run -it -d -p 25565:25565 --name folia-1.20.6 -e MINECRAFT_EULA=true blackao/folia:1.20.6
```
...
```bash
docker build --build-arg VERSION=1.20.6 --build-arg BUILD=latest -t blackao/folia:1.20.6 versions/1.20.6/
```
